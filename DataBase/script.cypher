...
CREATE (investigador)-[:PARTICIPA_EN]->(proyecto);
...
CREATE (publicacion)<-[:PERTENECE]-(proyecto);
...
MATCH (i:Investigador {nombre_completo: 'Nombre Investigador'})-[:PARTICIPA_EN]->(p:Proyecto)
...
RETURN p.idPry, p.anno_inicio, p.duracion_meses, p.area_conocimiento AS proyecto,
       [(i:Investigador)-[:PARTICIPA_EN]->(p) | {nombre_completo: i.nombre_completo, titulo_academico: i.titulo_academico, institucion: i.institucion, email: i.email}] AS investigadores,
       [(pub:Publicacion)<-[:PERTENECE]-(p) | {titulo_publicacion: pub.titulo_publicacion, anno_publicacion: pub.anno_publicacion, nombre_revista: pub.nombre_revista}] AS publicaciones;
...
MATCH (i:Investigador)-[:PARTICIPA_EN]->(p:Proyecto)
...
RETURN proyecto.anno_inicio AS anno_inicio,
       ...
       [(investigador)-[:PARTICIPA_EN]->(proyecto) | {
       ...
       [(publicacion)-[:PERTENECE]->(proyecto) | {
...
OPTIONAL MATCH (publicacion)<-[:PERTENECE]-(proyecto:Proyecto)
...
WITH proyecto
MATCH (publicacion:Publicacion)<-[:PERTENECE]-(proyecto)
...
MATCH (investigador)-[:PARTICIPA_EN]->(proyecto:Proyecto)<-[:PARTICIPA_EN]-(colega:Investigador)
